...
import { parseJwt, generateToken } from "../utils/jwt.js";
...
class SessionController {
  ...
  login = async (req, res) => {
    ...
    if (!email || !password) {
      return res.status(401).json({ status: "error", message: "Datos incompletos" });
    }
    ...
    if (!okPass) {
      return res.status(401).json({ status: "error", message: "No coinciden las credenciales" });
    }
  ...
  register = async (req, res) => {
    ...
    const userFound = await this.service.getUserEmail(email);
    if (userFound) {
      return res.status(401).json({ status: "error", message: "Usuario existente" });
    }
    ...
    if (!result) {
      return res.status(401).json({ status: "error", message: "No se completó el registro" });
    }
  ...
  restablecer = async (req, res) => {
    ...
    if (!ifresult) {
      return res.status(401).json({ status: "error", message: "Usuario inexistente" });
    }
    const tokenreset = generateToken({ email });
    ...
    if (result) return res.json({ status: "success", message: "Email enviado a su casilla" });
    return res.status(500).json({ status: "error", message: "No se pudo completar el envío" });
  };
  ...
  resetpassword = async (req, res) => {
    ...
    try {
      ...
      if (okPass) {
        return res.status(400).json({ message: "No puede utilizar la misma contraseña" });
      }

      const newUser = { ...user, password: createHash(newpassword) };
      await this.service.updateUser(user._id, newUser);
...
console.log("ME FUI")
  ...
  funcPassport = async (req, res) => {
    return res.json({ message: "Datos sensibles", user: req.user });
  };
